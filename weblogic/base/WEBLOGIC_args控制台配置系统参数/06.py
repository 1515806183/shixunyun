...
def run():
    ...
    if "<administration-port-enabled>true</administration-port-enabled>".lower().replace(" ", "") in com_ret\
            and "<administration-port>8000</administration-port>".lower().replace(" ", "") in com_ret:
        f.write("%s:%s正确 ---ok\n" % (test_name, test_vlu2))
    else:
        f.write("%s:%s错误 ---error\n" % (test_name, test_vlu2))
